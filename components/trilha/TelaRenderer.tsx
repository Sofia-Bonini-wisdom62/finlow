...
interface Props {
  tela: TelaData
  sessao: SessaoFluxo
  onQuizRespondido: (acertou: boolean) => void
  onInputMudou: (valores: SessaoFluxo) => void
}

export function TelaRenderer({ tela, sessao, onQuizRespondido, onInputMudou }: Props) {
  switch (tela.tipo) {
    case "conceito":
    ...
    case "quiz":
      return <TelaQuiz conteudo={tela.conteudo as Parameters<typeof TelaQuiz>[0]["conteudo"]} onRespondido={onQuizRespondido} />
    case "input":
