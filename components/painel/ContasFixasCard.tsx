  ...
  return (
    <div className="rounded-2xl bg-finlow-card p-5">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold uppercase tracking-wider text-finlow-muted">Contas Fixas</span>
        <div className="flex items-center gap-1">
          {contas.length > 0 && (
            <button
              onClick={() => setGerenciando(!gerenciando)}
              className="rounded-full px-3 py-1.5 text-xs font-medium text-finlow-muted transition-colors hover:text-finlow-text"
            >
      ...
      {contas.length === 0 ? (
        <p className="mt-3 text-sm text-finlow-muted">Nenhuma conta fixa cadastrada.</p>
      ) : (
          ...
          {contas.map((c) => (
            <li key={c.id} className="flex items-center justify-between text-sm">
              <div>
                ...
                {c.diaVencimento && (
                  <span className="ml-2 text-xs text-finlow-muted">dia {c.diaVencimento}</span>
                )}
              </div>
              <div className="flex items-center gap-2">
                <span className="text-finlow-muted">{brl(c.valor)}</span>
                {gerenciando && (
                  <>
