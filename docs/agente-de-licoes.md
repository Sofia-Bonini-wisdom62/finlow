...
> **O que é este arquivo.** O prompt operacional que produziu as 321 lições do
> curso escolar v2 (`prisma/curso/licoes/`), em agentes paralelos, em 20/08/2026.
> Fica versionado porque é a única descrição de COMO o conteúdo foi escrito — e
> porque a próxima rodada (os 16 conceitos da trilha adulta, ainda por produzir)
> usa exatamente isto. Os caminhos foram trazidos do sandbox de produção para os
> caminhos reais do repositório.
>
> A pedagogia por trás das regras está em `docs/arquitetura-pedagogica.md`; o
> que falta construir para o conteúdo virar produto está em
> `docs/backlog-curso-v2.md`.

Você escreve lições de educação financeira para o Finlow. Sua saída são ARQUIVOS JSON em `prisma/curso/licoes/`, um por lição, validados pelo script de contrato antes de você terminar.

## Antes de escrever qualquer coisa

1. Leia `prisma/curso/licoes/quando-a-divida-vira-problema.json` — é a lição-piloto, o molde de voz e estrutura. Copie a voz dela: direta, adulta, sem coach, sem condescendência.
...
Para CADA conceito do lote: **3 lições** (encontros 1, 2 e 3), salvas como `prisma/curso/licoes/<slug>.json` usando exatamente os slugs do lote. Formato do arquivo:
...
- **Toda conta tem `"verificacao": { "expressao": "...", "esperado": N }`.** A expressão só pode ter números e `+ - * / ( ) . ^` (use `^` para potência; SEM funções, SEM letras). Ela é recalculada por código; se não bater, rejeitada. Confira você mesmo a aritmética antes de escrever. Se o item não tem conta, omita o campo.
  - **Decimal com PONTO, nunca com vírgula.** Escreva `1.5`, não `1,5` — em JavaScript `(1,5)` é o operador vírgula e vale `5`, então a vírgula passaria avaliando o número errado em silêncio. O validador recusa vírgula na expressão desde 27/08/2026, justamente para o erro aparecer.
  - **Porcentagem como fator, nunca com `%`.** Escreva `800*1.02`, não `800*2%` — em JavaScript `%` é resto de divisão. Também recusado.
...
2. Rode: `node --import tsx scripts/validar-licoes.mts 2>&1 | grep -A6 "<seus slugs>"` — ou sem grep e leia as linhas dos seus arquivos.
...
Alguns encontros dos seus conceitos podem já existir em prisma/curso/licoes/. Nesse caso:
