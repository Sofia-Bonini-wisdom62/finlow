...
| Cadastro sem palavra de dentro do app | ✅ entregue em 20/08/2026 | Metade do item 4 da avaliação UX (a outra metade são as chaves OAuth na Vercel, que não são código). O subtítulo do cadastro dizia "Pra salvar seu perfil e seu progresso na **trilha**" e o rótulo do apelido perguntava "como você quer aparecer na **liga**": as duas palavras só existem depois do cadastro, e a landing que trouxe a pessoa até ali não usa nenhuma delas. Viraram "É grátis e leva menos de um minuto. A conta guarda seus números, suas conversas com a IA e o que você aprender por aqui" e "aparecer no ranking" — as mesmas promessas, sem exigir vocabulário. As telas continuam se chamando Trilha e Liga lá dentro, e quem chega por convite de escola segue lendo "ranking da sala". Guardado por `scripts/testar-landing.mts`, sem banco: varre a copy de `/cadastro` e `/login` atrás de jargão (trilha, liga, ofensiva, XP, combo, Finlo Coins, baú) olhando só o que vira texto na tela, nunca comentário nem nome de variável. Conferido contra o código mutilado nas duas frases. |
