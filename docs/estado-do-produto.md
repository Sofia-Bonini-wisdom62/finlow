...
| As portas de conta ganham teto de tentativa | ✅ entregue em 31/08/2026 | Primeira metade de *Recuperação de senha* (backlog, 30/08), e a que não depende de qual provedor de e-mail for escolhido — mesmo formato da *Conferência de duplicata* no Open Finance. **Era defeito de hoje:** `/login` aceitava senha errada sem conta nenhuma (nada olhava quantas vezes aquele e-mail já tinha errado) e `/api/cadastro` respondia "esse email já tem conta" quantas vezes se perguntasse, o que é a consulta pública de quem usa o Finlow — a decisão 2 do item, aparecendo no formulário que já existia. O teto **não fecha esse oráculo, encarece**: sem o 409 quem já é cliente fica preso num formulário que falha sem dizer por quê, e está escrito na rota para ninguém "consertar" isso escondendo a mensagem. Os números moram em `lib/portas-de-conta.ts` (folha, sem framework, pelo motivo de `lib/ops-lista.ts`): 10 falhas por conta e 30 por IP em 15 min, 10 cadastros por IP por hora. **Só a FALHA conta**, e o acerto zera — daí o par `excedeu` (pergunta, não registra) + `registrar` + `limpar` ao lado do `permitido` do lead: sem o `limpar`, dez erros de digitação seguidos de um acerto trancariam a pessoa DEPOIS de ela provar quem é, e o conserto óbvio de quem esbarrasse nisso seria desligar o teto. **IP desconhecido desliga a regra por IP** em vez de virar chave "anon" — balde compartilhado numa porta de login é queda geral no primeiro proxy que esconder o cabeçalho. Barrado devolve o mesmo que senha errada (o fluxo de credenciais do NextAuth não distingue, e dizer entregaria o relógio do teto), e por isso `/login` passou a dizer que a entrada esfria sozinha. Guardado por `scripts/testar-portas.mts`, sem banco e sem build, com seis conferências que leem o CÓDIGO das duas portas — a principal exige o teto **antes** do bcrypt, porque perguntar depois não nega ao atacante justamente o custo que se quer negar. Conferido contra o código mutilado: nove mutações, nove pegas. Onde: `lib/portas-de-conta.ts`, `lib/limite-taxa.ts`, `lib/auth.ts`, `app/api/cadastro/route.ts`, `app/(auth)/login/page.tsx` |
...
- **Chat e extrato continuam sem limite de TAXA** — o que entrou em 10/08 foi um
  ...
  extrato, que não tem guard nenhum. ~~O limitador (`lib/limite-taxa.ts`) segue
  só na rota de lead B2B.~~ Desde 31/08/2026 o limitador cobre também `/login` e
  `/api/cadastro` (ver a linha abaixo); **chat e extrato seguem de fora**, e são
  os que custam dinheiro.
- **Recuperação de senha não existe** (backlog, 30/08/2026): sem "esqueci minha
  senha", sem rota, sem caminho de suporte no produto. O que trava é não haver
  envio de e-mail no repositório — escolher provedor é decisão de produto, com
  contrato, domínio verificado e custo por mensagem, o mesmo formato do Open
  Finance. **Duas correções de fato entraram em 31/08:** (1) não existe cifra com
  chave derivada por usuário — é uma `ENCRYPTION_KEY` de ambiente com o usuário
  como AAD (`lib/cripto.ts`), então trocar a senha devolve a conta inteira, dados
  financeiros inclusive; (2) a reposição manual por `/ops` já existe e roda desde
  18/08, mas só para **membro de escola** (`redefinirSenha` exige vínculo em
  `MembroEscola`). Estendê-la à conta B2C é decisão de produto em aberto — a
  única coisa entre o cliente comum e o "perdeu a conta para sempre".
