...
| 9.2 | Reposição de senha de conta QUALQUER no `/ops`, e a saída de quem esqueceu no `/login` | ✅ 06/09 | `lib/ops-conta.ts` (`decidirReposicao` puro, `procurarConta`, `reporSenhaDeConta`), `GET /api/ops/contas`, `POST /api/ops/contas/senha`, `app/ops/contas`, `components/ops/ReporSenhaDeConta.tsx`, `app/(auth)/recuperar-senha`, link em `app/(auth)/login`. A rede de segurança do item *Recuperação de senha* do backlog: a reposição de 18/08 só alcançava membro de escola, e faltava o adulto sem escola por trás. Bateria vai a 74 conferências |
...
| Recuperação de senha: a rede de segurança, não o recurso | 🟡 metade entregue em 06/09/2026 | O item do backlog (30/08) está **parado numa decisão que não se toma dentro do repositório**: não há envio de e-mail aqui, e escolher provedor traz domínio verificado, SPF/DKIM, custo por mensagem e credencial de produção. O que entrou é a metade que não depende disso, e que tira o "perdeu a conta para sempre" da mesa: `/login` ganhou **"Esqueci minha senha"**, `/recuperar-senha` explica os três caminhos, e a operação repõe a senha de **qualquer** conta em `/ops/contas` (a reposição de 18/08 só alcançava membro de escola — ela nasceu para a criança do lote, cujo login `.invalid` não recebe mensagem — e faltava o adulto sem escola, que é a maior parte da base). **A tela pública não tem formulário, de propósito:** sem envio, "enviamos o link" é mentira e "não achei essa conta" vira consulta pública de quem usa o Finlow, que a decisão 2 do item recusa. **Conta do Google é recusada por padrão** e mandada para o botão do Google, com um segundo clique para quem perdeu também a conta do Google; a tela da escola decide o contrário porque lá o vínculo já é a autorização. **Membro de escola delega para `redefinirSenha`**, para o tamanho da senha não passar a depender de qual tela a operadora abriu. ⚠️ **Trocar a senha não derruba sessão aberta** (JWT): reposição devolve o acesso de quem perdeu, não expulsa ninguém, e isso está dito na tela. Guardado por `scripts/testar-ops.mts`, que vai a 74 conferências, sem banco e sem build, conferido contra código mutilado; a exigência de a tela pública seguir sem formulário é amarrada ao `package.json`, não à data, e afrouxa sozinha no dia em que o provedor entrar. Onde: `lib/ops-conta.ts`, `app/api/ops/contas`, `app/ops/contas`, `app/(auth)/recuperar-senha`, `app/(auth)/login/page.tsx` |
...
- **"Esqueci minha senha" automático continua não existindo**, e o que trava é
  escolher provedor de envio de e-mail — a mesma lacuna dos "alertas que avisam
  antes". O que entrou em 06/09/2026 é a rede de segurança pela operação
  (`/ops/contas`), com o aviso no `/login` que a torna pedível. Duas coisas
  seguem em aberto no item do backlog: o **teto de requisições** da futura rota
  pública (hoje `lib/limite-taxa.ts` cobre só o lead B2B) e **derrubar as
  sessões abertas** ao trocar a senha, que a sessão JWT não faz de graça.
