...
| 9.2 | Reposição de senha para **qualquer** conta (`/ops/contas`) + a porta `/esqueci-senha` no login | ✅ 05/09 | `lib/senha.ts` (régua da senha, o teto de 72 BYTES do bcrypt e `caminhoDeRecuperacao`), `lib/ops-usuario.ts` (`procurarContas`, `redefinirSenhaDaConta` — o hash nunca é selecionado), `GET /api/ops/usuarios`, `POST /api/ops/usuarios/[userId]/senha`, `components/ops/BuscaDeContas.tsx`, `app/(auth)/esqueci-senha/page.tsx`, `scripts/testar-senha.mts` (68 conferências). A reposição da escola (9.1) delega a escrita, e cadastro e troca de senha passam a usar a MESMA régua. **Não é o autoatendimento** — esse segue travado na escolha do provedor de e-mail; ver `backlog-produto.md`, *Recuperação de senha* |
