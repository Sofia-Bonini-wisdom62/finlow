...
| 13 | "Nova base de lições" (321 lições, 6 formatos, 20/08/2026) | `docs/backlog-produto.md`, seção "Nova base de lições" | Código pronto em 30/08/2026 (schema, `ItemLicao`, `ItemFlow`, transformação, seed) e verificado no preview — **nunca rodou em produção.** Falta `pnpm db:push` + `scripts/semear-licoes.mts --aplicar`, na ordem de sempre (coluna → deploy → seed). Até lá, os 24 módulos de EM e o pacote de EF continuam sendo a trilha escolar servida. |
