...
| 13 | Curso escolar v2 "substitui a trilha escolar atual" | `docs/arquitetura-pedagogica.md` §9, `docs/backlog-curso-v2.md` | Conteúdo (24/08), player (30/08, `components/curso/`, `/curso/[slug]`) e seed em produção (30/08: 107 conceitos, 321 lições, 5.268 itens) prontos; a piloto rodou ponta a ponta no ar. **Ainda não substituiu nada:** a fusão com a trilha (corredor, XP, progresso, caixas de Leitner) é fila própria (backlog itens 2 a 8). Até lá, quem abre a Trilha continua vendo os 107 módulos escolares antigos, e o curso v2 só é alcançável pela URL `/curso/<slug>`. |
...
fechada). Das duas divergências que importavam, uma fechou:

- ~~**A exportação LGPD não exporta tudo**~~ ✅ **24/08/2026** — passou a
  exportar, inclusive conversas e memórias, que eram o mais sensível do que
  faltava (divergência 6).
