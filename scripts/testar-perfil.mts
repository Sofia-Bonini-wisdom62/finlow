...
// Nenhum número saiu da tela quando o dinheiro entrou na frente deles.
for (const tile of ["DIAS SEGUIDOS", "XP TOTAL", "PRECISÃO"]) {
  checar(`o tile ${tile} continua na tela`, tela.includes(tile))
}
// Os tiles do protótipo v2 continuam onde o desenho os pôs — logo abaixo do
// cabeçalho — e o dinheiro entra ANTES deles. Descê-los para junto da porta da
// Liga era a outra saída, e resolveria o mesmo problema; mas mover elemento que
// um desenho registrado posicionou é decisão de produto, e o pedido do backlog
// ("nome e foto no topo, o dinheiro logo abaixo") se cumpre sem ela.
// `indexOf` devolve -1 para o que não existe, e -1 é menor que qualquer
// posição: sem exigir presença, a ordem passaria numa tela que perdeu a seção.
const iTiles = tela.indexOf('aria-label="Seu jogo"')
checar("a seção dos tiles existe", iTiles > 0)
checar("o dinheiro vem antes dos tiles do jogo", iDinheiro > 0 && iDinheiro < iTiles, `${iDinheiro} < ${iTiles}`)
checar("e os tiles continuam antes da rosca", iTiles > 0 && iRosca > 0 && iTiles < iRosca, `${iTiles} < ${iRosca}`)

// ---------- a tela degrada em vez de quebrar ----------
//
// `dinheiro` é campo NOVO na resposta. Uma resposta velha em cache não o traz,
// e ler `.saldo` de `undefined` derruba a árvore React inteira — o retrato
// some por causa de um bloco acessório.
console.log("\nA TELA DEGRADA EM VEZ DE QUEBRAR")
checar("`dinheiro` é opcional no tipo da resposta", /dinheiro\?:/.test(tela))
checar(
  "a tela lê o campo com guarda (`?? `), nunca cru",
  /perfil\.dinheiro \?\?/.test(tela) && !/perfil\.dinheiro\.[a-z]/i.test(tela),
  ""
)
