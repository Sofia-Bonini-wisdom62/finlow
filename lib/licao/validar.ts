// Finlow — validador de contrato de lição. O seed ABORTA se qualquer limite estourar.
import { CUSTO, LIMITES, composicaoDe, custoLicao, type Formato, type Papel } from "./formatos";

export type Item = {
  formato: Formato; papel: Papel; conceitoId: string; dificuldade?: number;
  criterio?: string; enunciado?: string; pergunta?: string; instrucao?: string;
  resposta?: boolean; ancora?: string; feedbackErro?: string;
  alternativas?: { texto: string; correta: boolean; feedbackErro?: string | null }[];
  itens?: { texto: string; caixaCorreta?: string; posicaoCorreta?: number; feedbackErro?: string }[];
  caixas?: { id: string; rotulo: string }[];
  campo?: string; min?: number; max?: number; gabarito?: number; toleranciaPct?: number;
  fonte?: string; feedbackPerto?: string; feedbackLonge?: string;
  /** Obrigatório em todo item cujo gabarito vem de uma conta. */
  verificacao?: { expressao: string; esperado: number; tolerancia?: number };
};

export type Licao = { slug: string; segmento: string; conceitoPrincipal: string; telas: Item[]; reserva: Item[] };

const SEGURO = /^[0-9+\-*/().,\s%^]+$/; // sem letras: nenhuma expressão pode chamar função
...
export function validarLicao(l: Licao): string[] {
  ...
  l.telas.forEach((t, i) => {
    ...
    if (esperado && t.papel !== esperado.papel) push(`tela ${i + 1}: papel ${t.papel}, esperado ${esperado.papel}`);
  ...
  for (const [i, t] of l.telas.entries()) {
    ...
    switch (t.formato) {
      ...
      case "fecho": {
        ...
        if (alts.length !== LIMITES.alternativasEscolha3) push(`${n}: ${alts.length} alternativas, exigido 3`);
        if (alts.filter((a) => a.correta).length !== 1) push(`${n}: precisa de exatamente 1 correta`);
        alts.forEach((a, j) => {
          if (a.texto.length > lim) push(`${n}: alternativa ${j + 1} com ${a.texto.length} car., limite ${lim}`);
      ...
      case "ordenar": {
        ...
        if (t.formato === "classificar" && (t.caixas?.length ?? 0) !== 2) push(`${n}: precisa de 2 caixas`);
        if (!t.feedbackErro && !its.every((i) => i.feedbackErro)) push(`${n}: sem feedback de erro`);
        respostas += its.length; break;
      }
      case "estimativa":
  ...
  const minResp = l.segmento === "ef12" ? 10 : LIMITES.respostasMin;
  if (respostas < minResp) push(`${respostas} respostas avaliáveis, mínimo ${minResp}`);
  if (l.reserva.length < LIMITES.itensReserva) push(`${l.reserva.length} itens de reserva, mínimo ${LIMITES.itensReserva}`);

  // Reserva não pode repetir pergunta da lição (senão a revisão mede memória da resposta),
  // e precisa variar de formato para forçar transferência. Seção 5 do doc de arquitetura.
  const textosLicao = new Set(l.telas.map((t) => (t.enunciado ?? t.pergunta ?? "").trim()).filter(Boolean));
  ...
  if (new Set(l.reserva.map((r) => r.formato)).size < 2) push(`reserva precisa de ao menos 2 formatos distintos`);
